 ...
 si_press : function()
{
   ...
   //if units.uu=2 then result:=(14.5/10);
 ...
 Drawx : function(x,y,cc,context)
{
 ...
 var context = canvas.getContext("2d");
  ...
  // if units.uu=2 then rad:=rad*25.4;  //inch to mm
   ...
   veloc=this.Nodes[n2].FlowIn; //*0.063;   //l/s
   
   area = Math.PI * ( (rad/2) * (rad/2) ) / 1000;
   veloc =veloc/area;    //m/s
   
   //if Units.uu=2 then Veloc:=Veloc/feet;


   this.flow = this.Nodes[n2].FlowIn * 1;
   
   if (mani) 
   {
      
      if (this.flow>this.highflowMan)  this.highflowMan =this.flow;
      if (veloc>this.highvelocMan)  this.highvelocMan =veloc;
	}
	else
	{
      if (this.flow>this.highflowLat)  this.highflowLat=this.flow;
      if (veloc>this.highvelocLat)  this.highvelocLat=veloc;
	}
